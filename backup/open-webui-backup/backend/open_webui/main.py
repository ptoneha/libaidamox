        ...
        if scope["type"] == "http":
            ...
            async def wrapped_send(message):
                if message["type"] == "http.response.start":
                    ...
                    # 确保移除任何已存在的CORS头，以避免重复
                    headers = [(k, v) for k, v in headers if k != b"access-control-allow-origin"]
                    # 添加允许所有来源的CORS头
...
# SPA静态文件处理类，同时支持CORS和SPA特性
class CORSSPAStaticFiles(CORSStaticFiles):
    async def get_response(self, path: str, scope):
        try:
            return await super().get_response(path, scope)
        except (HTTPException, StarletteHTTPException) as ex:
...
if os.path.exists(FRONTEND_BUILD_DIR):
    ...
    app.mount(
        "/",
        CORSSPAStaticFiles(directory=FRONTEND_BUILD_DIR, html=True),
